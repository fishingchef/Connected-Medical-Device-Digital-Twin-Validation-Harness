...
Endpoints:
  ...
  POST /api/scenarios/run         run a full simulation
  GET  /api/runs/{run_id}         get run status + summary
  GET  /api/runs/{run_id}/packets get all ingested packets
  GET  /api/runs/{run_id}/report  get validation report
  GET  /api/runs                  list recent runs
  POST /api/ingest                mock cloud ingest endpoint
"""
...
from db.models import (
    IngestedPacket, SimulationRun, ValidationResult,
    create_tables, get_db
)
from core.generators.physiology import NAMED_SCENARIOS, ScenarioConfig, PhysiologyGenerator
...
app = FastAPI(
    ...
    version="0.1.0",
    description="End-to-end connected device simulation and validation platform",
)
...
class RunScenarioRequest(BaseModel):
    ...
    # Gateway fault schedule — overrides fault_profile outage window
...
def list_scenarios():
    return {
        "scenarios": [
            {
                "id": "GW_WIFI_OUTAGE_01",
                "name": "Gateway Wi-Fi outage with delayed upload",
            ...
            {
                "id": "HIGH_MOTION_01",
                "name": "High motion — degraded signal",
...
def run_scenario(req: RunScenarioRequest, db: Session = Depends(get_db)):
    ...
    # Build components
    scenario_fn  = NAMED_SCENARIOS[req.scenario_id]
    phys_config  = scenario_fn()
    generator    = PhysiologyGenerator(phys_config)
    samples      = list(generator.generate())

    wearable_cfg = WearableConfig(
        firmware_version=fw,
        initial_battery_pct=req.initial_battery_pct,
        ambient_temp_c=req.ambient_temp_c,
    )
    wearable = WearableSimulator(wearable_cfg)

    # Gateway fault schedule
    fault_schedule = []
    ...
    elif req.fault_profile in ("outage_20min",):
        fault_schedule = [(2400, 3600, "WIFI_DOWN")]

    gw_config = GatewayConfig(fault_schedule=fault_schedule)
    gateway   = GatewaySimulator(gw_config)

    # Network injector
    fault_prof = FAULT_PROFILES.get(req.fault_profile, FAULT_PROFILES["clean"])
    injector   = NetworkFaultInjector(fault_prof)
    ...
    def upload_fn(packets):
        ...
        seen_ids = set()
        for p in packets:
            if p.packet_id in seen_ids:
                p_dict = p.to_dict()
                p_dict["duplicate"] = True
                continue
            seen_ids.add(p.packet_id)
        ...
        for p in accepted:
            p_dict = p.to_dict()
            sample_ts = p_dict.get("sample_timestamp", "")
            received  = p_dict.get("received_at", "")

            # Calculate ingestion delay
            try:
                from datetime import datetime as dt
                s = dt.fromisoformat(sample_ts.replace("Z", "+00:00"))
                r = dt.fromisoformat(received.replace("Z", "+00:00"))
                delay_sec = (r - s).total_seconds()
            except Exception:
                delay_sec = 0.0
            ...
            row = IngestedPacket(
                ...
                received_at      = p_dict["received_at"],
                elapsed_sec      = p_dict["elapsed_sec"],
                motion           = p_dict["motion"],
                hr_bpm           = p_dict["hr_bpm"],
                rr_rpm           = p_dict["rr_rpm"],
                temp_c           = p_dict["temp_c"],
                signal_confidence= p_dict["signal_confidence"],
                activity_label   = p_dict["activity_label"],
                battery_pct      = p_dict["battery_pct"],
                firmware_state   = p_dict["firmware_state"],
                ambient_temp_c   = p_dict["ambient_temp_c"],
                ble_rssi_dbm     = p_dict["ble_rssi_dbm"],
    ...
    for cr in report.results:
        row = ValidationResult(
            ...
            risk_id=cr.risk_id,
            check_name=cr.check_id,
            expected=str(cr.expected),
            actual=str(cr.actual),
            passed=cr.passed,
    ...
    return {
        ...
        "gateway_events": gw_summary["events"][:20],   # first 20 for preview
    }
...
def get_packets(run_id: str, limit: int = 200, db: Session = Depends(get_db)):
    pkts = (
        db.query(IngestedPacket)
        .filter(IngestedPacket.run_id == run_id)
        .order_by(IngestedPacket.elapsed_sec)
        .limit(limit)
        .all()
    )
    return {
        ...
        "packets": [
            {
                ...
                "sample_timestamp": p.sample_timestamp,
...
def get_report(run_id: str, db: Session = Depends(get_db)):
    results = (
        ...
        .filter(ValidationResult.run_id == run_id)
        .all()
    )
    if not results:
